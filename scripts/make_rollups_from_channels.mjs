*** a/scripts/make_rollups_from_channels.mjs
--- b/scripts/make_rollups_from_channels.mjs
@@
...
+// Extra: common “short” tells seen in RSS titles (emoji, micro-captions)
+const MICRO_RE = /\b(60s|30s|reel[s]?|clip|snippet)\b/i;
@@
...
+// CSV parsing that tolerates commas inside quotes
...
+    // Strict rollups: without durations we cannot trust long-form → drop later
...
+      // Preserve batch but keep undefined duration; strict gate will drop them
+      out.push(...batch.map((v) => ({ ...v, latest_video_duration_sec: undefined, view_count: undefined })));
     }
     await sleep(100);
   }
...
+  // STRICT: require KNOWN duration >= 11 min. If duration unknown → DROP.
+  // This is the key reason Shorts (e.g. 2–6 min) won’t appear when API fails.
