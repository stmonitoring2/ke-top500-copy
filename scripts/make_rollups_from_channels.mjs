...
async function enrichWithYouTubeAPI(items) {
  ...
  if (!apiKey || !items.length) {
    console.warn("[rollup] YT_API_KEY missing or no items → durations unknown; strict gate will drop all.");
    ...
    for (let attempt = 0; attempt <= API_RETRIES; attempt++) {
      try {
        ...
        if (attempt === API_RETRIES) {
          console.error("[rollup] API error (final):", e.message);
        } else {
          console.warn("[rollup] API error, retrying:", e.message);
          await sleep(400);
          continue;
        }
      }
...
async function main() {
  ...
  // Early exit with placeholder if no candidates at all
  ...
  // STRICT: require known duration >= 11min (if unknown, DROP to prevent shorts)
